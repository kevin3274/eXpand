...
namespace Xpand.ExpressApp.FilterDataStore.Core {
    public static class FilterProviderManager {
        ...
        public static FilterProviderBase GetFilterProvider(Type objectType, string filterMemberName, StatementContext modify) {
            FilterProviderBase provider = Providers.Cast<FilterProviderBase>().Where(
                probase => (probase.ObjectTypes == null || probase.ObjectTypes.Any(x=>objectType.IsAssignableFrom(x))) && probase.FilterMemberName == filterMemberName && (probase.StatementContext == modify || probase.StatementContext == StatementContext.Both)).FirstOrDefault();
